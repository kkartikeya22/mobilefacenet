...
def getThreshold(scores, flags, thrNum):
    accuracies = np.zeros((2 * thrNum + 1, 1))
    thresholds = np.arange(-thrNum, thrNum + 1) * 1.0 / thrNum
    for i in range(2 * thrNum + 1):
        accuracies[i] = getAccuracy(scores, flags, thresholds[i])

    max_index = np.squeeze(accuracies == np.max(accuracies))
...
def getFeatureFromTorch(lfw_dir, feature_save_dir, resume=None, gpu=True):
    ...
    for data in lfw_loader:
        if gpu:
            data = [[d.cuda() for d in img] for img in data]
        count += data[0].size(0)
        print('extracting deep features from the face pair {}...'.format(count))
        res = [net(d).data.cpu().numpy() for img in data for d in img]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', type=str, default='./model/best/068.ckpt', help='The path of the saved model')
    parser.add_argument('--feature_save_dir', type=str, default='./result/best_result.mat', help='The path to save extracted features, must be .mat file')
